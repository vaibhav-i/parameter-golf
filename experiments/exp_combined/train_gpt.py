...
class Hyperparameters():
    ...
    # Streaming log-bias eval (Nacrith arXiv:2602.19626)
    log_bias_enabled = bool(int(os.environ.get('LOG_BIAS_ENABLED', '1')))
    log_bias_lr = float(os.environ.get('LOG_BIAS_LR', '0.001'))
...
def eval_val_sliding_log_bias(
    h: Hyperparameters,
    device: torch.device,
    val_data: ValidationData,
    base_model: nn.Module,
    log_bias_lr: float = 0.001,
    reset_per_window: bool = False,
) -> tuple[float, float]:
    """Streaming online log-bias correction (Nacrith arXiv:2602.19626).

    Maintains b in R^vocab. After scoring each token at position t (true token x_t):
      b += lr * (one_hot(x_t) - softmax(logits_t + b))
    Applied causally: b is only updated AFTER the token is scored.
    """
    base_model.eval()
    logits_fn = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=True)
    ...
    window_starts = sorted([ws for ws in range(0, total_tokens, h.eval_stride)
                            if ws + context_size < total_tokens])
    total_windows = len(window_starts)
    my_s = (total_windows * h.rank) // h.world_size
    my_e = (total_windows * (h.rank + 1)) // h.world_size
    ...
    # Online bias vector — persists across windows (reset_per_window=False is recommended)
    ...
    with torch.no_grad():
        for ws in my_windows:
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = logits_fn(x)  # [1, wlen, vocab]
            logits = logits[0].float()  # [wlen, vocab] — float32 for bias arithmetic

            # Scored region starts at context_size for non-first windows
            s = 0 if ws == 0 else context_size

            for t in range(wlen):
                # Apply bias to get adjusted logits
                adj_logits = logits[t] + b  # [vocab]
                p = torch.softmax(adj_logits, dim=-1)  # [vocab]

                if t >= s:
                    # Score this token
                    true_tok = int(y[t].item())
                    nll_t = -torch.log(p[true_tok] + 1e-10)
                    loss_sum += nll_t.to(torch.float64)
                    token_count += 1.0
                    tb = float(val_data.base_bytes_lut[y[t]].item())
                    if val_data.has_leading_space_lut[y[t]] and not val_data.is_boundary_token_lut[x[0, t]]:
                        tb += 1.0
                    byte_count += tb

                # Update bias AFTER scoring (strictly causal)
                true_tok = int(y[t].item())
                b[true_tok] += log_bias_lr  # one_hot contribution
                b -= log_bias_lr * p        # subtract p_adjusted
...
def eval_val_sliding_causal_slot_with_log_bias(
    h: Hyperparameters,
    device: torch.device,
    val_data: ValidationData,
    base_model: nn.Module,
    ...
    slot_lr: float = 0.005,
    log_bias_lr: float = 0.001,
    log_bias_reset_per_window: bool = False,
) -> tuple[float, float]:
    """Combined: causal SLOT delta + streaming log-bias correction.

    Per window: optimize delta on context tokens (SLOT), then score the stride
    with the delta applied AND a streaming log-bias vector that updates causally
    ...
    # Streaming log-bias — persists across windows unless reset_per_window
    b = torch.zeros(vocab_size, dtype=torch.float32, device=device)

    for ws in my_windows:
        if log_bias_reset_per_window:
            b.zero_()
        ...
        with torch.no_grad():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                hidden = base_model.forward_hidden(x)
                hidden_adapted = hidden + delta.detach()
                logits_window = base_model.hidden_to_logits(hidden_adapted)
            logits_window = logits_window[0].float()  # [wlen, vocab]

        # Score the stride region with log-bias applied
        s = 0 if ws == 0 else context_size

        # Apply log-bias updates for context tokens (to keep b consistent)
        with torch.no_grad():
            for t in range(s):
                adj_logits = logits_window[t] + b
                p = torch.softmax(adj_logits, dim=-1)
                true_tok = int(y[t].item())
                b[true_tok] += log_bias_lr
                b -= log_bias_lr * p

            # Score and update log-bias for stride tokens
...
def train_and_eval(h: Hyperparameters, device: torch.device) -> None:
    ...
    # Choose final eval mode based on enabled flags:
    #   slot + log_bias: causal SLOT with log-bias applied in the scored stride
    #   slot only:       causal SLOT eval
    #   log_bias only:   streaming log-bias sliding window eval
    #   neither:         regular sliding window eval (if sliding_window_enabled)
    if h.slot_enabled and h.log_bias_enabled:
        log("eval_mode: causal_slot + log_bias (combined)")
        timed_eval(
            "combined_slot_log_bias",
            eval_val_sliding_causal_slot_with_log_bias,
            h, device, val_data, eval_model,
            slot_steps=h.slot_steps,
            slot_lr=h.slot_lr,
            log_bias_lr=h.log_bias_lr,
            log_bias_reset_per_window=h.log_bias_reset_per_window,
        )
    elif h.slot_enabled:
        log("eval_mode: causal_slot only")
        timed_eval(
            "causal_slot_sliding_window",
            eval_val_sliding_causal_slot,
            h, device, val_data, eval_model,
            slot_steps=h.slot_steps,
            slot_lr=h.slot_lr,
        )
    elif h.log_bias_enabled:
        log("eval_mode: log_bias only")
        timed_eval(
            "log_bias_sliding_window",
            eval_val_sliding_log_bias,
            h, device, val_data, eval_model,
            log_bias_lr=h.log_bias_lr,
            reset_per_window=h.log_bias_reset_per_window,
        )
    elif h.sliding_window_enabled:
        log("eval_mode: regular sliding window")
