...
def eval_val_sliding_causal_slot(
    ...
    slot_steps: int = 16,
    slot_lr: float = 0.005,
    batch_seqs: int = 8,  # smaller batch due to SLOT overhead
) -> tuple[float, float]:
    ...
    for bi in range(0, len(my_windows), batch_seqs):
        ...
        # --- SLOT optimization on context tokens ---
        # delta: [1, 1, model_dim] — broadcasts over batch and sequence
        ...
        for _ in range(slot_steps):
            slot_opt.zero_grad()
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                # Get hidden states and apply delta — context positions only
                hidden = base_model.forward_hidden(x_batch)  # [bsz, seq_len, model_dim]
                hidden_adapted = hidden + delta  # broadcast
                logits_ctx = base_model.hidden_to_logits(hidden_adapted)  # [bsz, seq_len, vocab]

            # Only compute loss on context region [0:context_size]
            ctx_logits = logits_ctx[:, :context_size, :].reshape(-1, logits_ctx.size(-1)).float()
            ctx_targets = y_batch[:, :context_size].reshape(-1)
        ...
        with torch.inference_mode():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                hidden = base_model.forward_hidden(x_batch)
